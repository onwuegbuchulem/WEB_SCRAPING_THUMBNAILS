...
def rename_thumbnails(mapping_file, input_folder, output_folder):
    ...
    for line in lines[1:]:
        ...
        if len(columns) == 2:
            ...
            output_path = os.path.join(output_folder, parent_filename.replace('.docx', '.bmp'))

            # Copy the file to the output folder with the new name
            os.rename(input_path, output_path)
            print(f'Renamed and moved: {thumbnail_filename} -> {parent_filename.replace(".docx", ".bmp")}')

# Example usage
mapping_file = 'mappingTableBMP/mapping.txt'
input_folder = 'Thumbnail48'
output_folder = 'MappedThumbnail48'
